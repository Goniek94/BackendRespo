...
export const getComments = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Błąd serwera podczas pobierania listy komentarzy." });
  }
};
...
export const getCommentDetails = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({
        message: "Błąd serwera podczas pobierania szczegółów komentarza.",
      });
  }
};
...
export const updateCommentStatus = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({
        message: "Błąd serwera podczas aktualizacji statusu komentarza.",
      });
  }
};
...
export const deleteComment = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Błąd serwera podczas usuwania komentarza." });
  }
};
...
export const bulkUpdateComments = async (req, res) => {
  try {
    ...
    if (!Array.isArray(commentIds) || commentIds.length === 0) {
      return res
        .status(400)
        .json({ message: "Nie podano poprawnej listy komentarzy." });
    }
    ...
    if (!["approved", "rejected"].includes(status)) {
      return res
        ...
        .json({
          message:
            "Nieprawidłowy status. Dozwolone wartości: approved, rejected.",
        });
    }
    ...
    return res.status(200).json({
      message: `Zaktualizowano status ${result.modifiedCount} komentarzy na "${
        status === "approved" ? "zatwierdzony" : "odrzucony"
      }".`,
    });
  } catch (error) {
    ...
    return res
      .status(500)
      .json({
        message: "Błąd serwera podczas masowej aktualizacji komentarzy.",
      });
  }
};
