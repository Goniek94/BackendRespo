...
adminActivitySchema.index({
  ...
  "securityFlags.suspiciousActivity": 1,
});
...
adminActivitySchema.virtual("actionCategory").get(function () {
  ...
  if (actionType.includes("login") || actionType.includes("security"))
    return "security";
  return "system";
});
...
adminActivitySchema.statics.findByResource = function (
  resourceType,
  resourceId
) {
  return this.find({
...
adminActivitySchema.post("save", function (doc) {
  ...
  if (doc.isHighRisk) {
    ...
    console.log(
      `HIGH RISK ACTIVITY: ${doc.actionType} by admin ${doc.adminId}`
    );
  }
