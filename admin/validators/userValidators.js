...
export const validateUserUpdate = [
  ...
  body('status')
    ...
    .withMessage('Status must be one of: active, blocked, pending, deleted'),
...
export const validateUserBlock = [
  ...
  body('blocked')
    .isBoolean()
    .withMessage('Blocked status must be a boolean'),
  ...
  body('updateData')
    ...
    .custom((value) => {
      ...
      if (invalidFields.length > 0) {
        throw new Error(`Invalid fields in bulk update: ${invalidFields.join(', ')}`);
      }
...
export const validateUserQuery = [
  ...
  query('status')
    .optional()
    .isIn(['active', 'blocked', 'pending', 'deleted'])
    .withMessage('Status filter must be one of: active, blocked, pending, deleted'),

  query('sortBy')
    .optional()
    .isIn(['createdAt', 'updatedAt', 'name', 'email', 'role', 'status'])
    .withMessage('Sort field must be one of: createdAt, updatedAt, name, email, role, status'),
