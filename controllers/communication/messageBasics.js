...
export const sendMessage = async (req, res) => {
  try {
    ...
    // Walidacja - sprawdź czy użytkownik nie próbuje wysłać wiadomości do samego siebie
    if (
      senderId === recipient ||
    ...
    if (req.files && req.files.length > 0) {
      ...
      try {
        ...
        console.log(
          `🔄 Uploading ${validation.files.length} images to Supabase for message ${savedTempMessage._id}`
        );
        ...
        console.log(
          `✅ Successfully uploaded ${uploadedImages.length} images for message ${savedTempMessage._id}`
        );
    ...
    try {
      ...
      if (shouldNotify) {
        console.log(
          `Wysyłam powiadomienie o nowej wiadomości od ${senderName} do ${recipientId}`
        );
        ...
        console.log(
          `Pomijam powiadomienie o wiadomości od ${senderName} do ${recipientId} - użytkownik jest aktywny w konwersacji lub otrzymał już powiadomienie`
        );
      }
