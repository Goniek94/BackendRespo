...
export const getUserDashboard = async (req, res) => {
  try {
    ...
    const [activeListingsCount, completedTransactionsCount, totalListingsCount, userAds] = await Promise.all([
      ...
      Ad.find({ owner: userId })
        .sort({ updatedAt: -1 })
        .limit(10)
        .select('_id title brand model price status images mainImageIndex createdAt updatedAt')
        .lean()
    ]);

    // Oblicz status pełnej weryfikacji
    const isFullyVerified = user.isEmailVerified && user.isPhoneVerified && user.isVerified;
    ...
    const dashboardData = {
      ...
      userListings: userAds.map(ad => ({
        ...
        mainImage: ad.images && ad.images.length > 0 ? ad.images[ad.mainImageIndex || 0] : null,
    ...
    res.status(500).json({
      success: false,
      message: 'Błąd serwera podczas pobierania danych dashboardu',
