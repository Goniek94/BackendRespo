...
import {
  sendVerificationCode as sendSMSAPICode,
  verifyCode as verifySMSAPICode,
} from "../../config/smsapi.js";
...
export const sendVerificationCode = async (req, res) => {
  ...
  try {
    ...
    if (MOCK_MODE) {
      ...
      // Wysłanie kodu przez SMSAPI (PRODUCTION MODE)
      const result = await sendSMSAPICode(phoneNumber, code);
...
export const verifyVerificationCode = async (req, res) => {
  ...
  try {
    ...
    const isCodeExpired =
      user.twoFACodeExpires && new Date(user.twoFACodeExpires) < new Date();
    ...
    // Sprawdź kod przez SMSAPI
    const verificationResult = await verifySMSAPICode(phoneNumber, code);
    ...
    if (MOCK_MODE) {
      ...
      try {
        if (phone) {
          ...
          const result = await sendSMSAPICode(phone, code);
          console.log("Wynik wysyłania kodu:", result);
        } else {
          console.log(
            "Brak numeru telefonu, kod powinien być wysłany przez email",
          );
...
export const verifySMSCodeAdvanced = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      new Date() > user.smsVerificationCodeExpires
    ) {
      console.log(
        "Kod SMS wygasł, czas wygaśnięcia:",
        user.smsVerificationCodeExpires,
      );
      return res.status(400).json({
        success: false,
    ...
    if (isTestCode || user.smsVerificationCode === code) {
      ...
      if (user.isEmailVerified) {
        ...
        console.log(
          "Rejestracja zakończona pomyślnie dla użytkownika:",
          user._id,
        );
      }
  ...
  try {
    ...
    if (isTestCode) {
      ...
      if (user) {
        ...
        await user.save();
        console.log(
          "Zaktualizowano status weryfikacji dla użytkownika:",
          user._id,
        );
      } else {
