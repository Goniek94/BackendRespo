...
const createAdFilter = (query) => {
  ...
  if (query.imported === "true" || query.imported === true)
    filter.imported = true;
  if (query.imported === "false" || query.imported === false)
    filter.imported = false;
  if (query.registeredInPL === "true" || query.registeredInPL === true)
    filter.registeredInPL = true;
  if (query.registeredInPL === "false" || query.registeredInPL === false)
    filter.registeredInPL = false;
  if (query.firstOwner === "true" || query.firstOwner === true)
    filter.firstOwner = true;
  if (query.firstOwner === "false" || query.firstOwner === false)
    filter.firstOwner = false;
  if (query.disabledAdapted === "true" || query.disabledAdapted === true)
    filter.disabledAdapted = true;
  if (query.disabledAdapted === "false" || query.disabledAdapted === false)
    filter.disabledAdapted = false;
  ...
  if (query.engineCapacityFrom || query.engineCapacityTo) {
    ...
    if (query.engineCapacityFrom)
      filter.engineSize.$gte = parseInt(query.engineCapacityFrom);
    if (query.engineCapacityTo)
      filter.engineSize.$lte = parseInt(query.engineCapacityTo);
  }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const matchingAds = await Ad.find(filter).select("brand model").lean();
      ...
      matchingAds.forEach((ad) => {
        if (ad.brand) {
          ...
          if (ad.model) {
            ...
            modelCounts[ad.brand][ad.model] =
              (modelCounts[ad.brand][ad.model] || 0) + 1;
          }
        }
      });
