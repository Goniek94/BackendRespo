...
const adValidationSchema = Joi.object({
  ...
  vin: Joi.string()
    ...
    .messages({
      "string.pattern.base":
        "Numer VIN musi składać się z 17 znaków (bez I, O, Q).",
    }),
  ...
  registrationNumber: Joi.string()
    ...
    .messages({
      "string.pattern.base":
        "Numer rejestracyjny może zawierać maksymalnie 8 znaków (litery i cyfry).",
    }),

  // purchaseOptions - akceptujemy różne formaty z frontendu i backendu
  purchaseOptions: Joi.string()
    .valid(
      // Standard sale options (frontend)
      "Sprzedaż",
      "sprzedaz",
      "sprzedaż",
      "Faktura VAT",
      ...
      "Umowa kupna-sprzedaży",
      // Standard sale options (backend - uppercase)
      "SPRZEDAŻ",
      "FAKTURA VAT",
      "INNE",
      // Rental options (frontend)
      "najem",
      "Najem",
      "Wynajem",
      "wynajem",
      // Rental options (backend - uppercase)
      "NAJEM",
      // Leasing options (frontend)
      "leasing",
      "Leasing",
      // Leasing options (backend - uppercase)
      "LEASING",
      // Cession options (cesja leasingu) (frontend)
      "cesja",
      "Cesja",
      "Cesja leasingu",
      "cesja leasingu",
      // Cession options (backend - uppercase)
      "CESJA LEASINGU",
      // Exchange options (zamiana) (frontend)
      "zamiana",
      "Zamiana",
      // Exchange options (backend - uppercase)
      "ZAMIANA"
    )
    .default("Sprzedaż")
    .messages({
  ...
  // negotiable
  negotiable: Joi.string().valid("Tak", "Nie").default("Nie").messages({
  ...
  sellerType: Joi.string()
    .valid("prywatny", "firma", "Prywatny", "Firma")
    .default("Prywatny")
    .messages({
  ...
  // Napęd - akceptujemy dowolny string (walidacja na frontendzie)
  ...
  // Nowe pola dla cesji - akceptujemy null i pusty string
  leasingCompany: Joi.string().allow("", null),
  ...
  // Nowe pola dla zamiany - akceptujemy null i pusty string
  exchangeOffer: Joi.string().allow("", null),
  exchangeValue: Joi.number().min(0).allow(null),
  exchangePayment: Joi.number().allow(null), // może być ujemna
  exchangeConditions: Joi.string().allow("", null),
  ...
  // Pola płatności (opcjonalne)
  paymentAmount: Joi.number().min(0).allow(null).optional(),
  paymentMethod: Joi.string()
    .valid("card", "blik", "transfer", "paypal", "przelewy24", "payu")
    .allow("")
    .optional(),
  transactionType: Joi.string()
    .valid("standard_listing", "featured_listing")
    .allow("")
    .optional(),
